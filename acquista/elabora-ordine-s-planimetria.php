<?php
    ...
    /* Ordinazione planimetria catastale */
    if(isset($_POST['ordina-s-planimetria-catastale'])) {
		...
		/* Sezione urbana ID */
		$riferimenti_catastali_su_id = $_POST['rif-catastali-su'];

		/* Recupero nome sezione urbana da ID */
		$query_recupero_nome_su = "SELECT su_name FROM tabella_su WHERE su_id = '$riferimenti_catastali_su_id'";
		$record_recupero_nome_su = $query_connection->query($query_recupero_nome_su);
		$campo_recupero_nome_su = $record_recupero_nome_su->fetch_assoc();
		$riferimenti_catastali_ord_su_db = addslashes($campo_recupero_nome_su['su_name']);

		$riferimenti_catastali_fgl = addslashes($_POST['rif-catastali-foglio']);
		$riferimenti_catastali_par = addslashes($_POST['rif-catastali-particella']);
		$riferimenti_catastali_sub = addslashes($_POST['rif-catastali-subalterno']);
		
		if(!$riferimenti_catastali_sub) {
			$riferimenti_catastali_sub = '0';
		}
		...
		/* Visura ordinaria */
		if(isset($_POST['ordine-planimetria-visura-ordinaria'])) {
		...
		/* Visura storica */
		if(isset($_POST['ordine-planimetria-visura-storica'])) {
		...
		/* Estratto di mappa */
		if(isset($_POST['ordine-planimetria-estratto-mappa'])) {
		...
		$ord_price = round($prezzo_planimetria_catastale + $prezzo_visura_ordinaria + $prezzo_visura_storica + $prezzo_estratto_mappa, 2);
